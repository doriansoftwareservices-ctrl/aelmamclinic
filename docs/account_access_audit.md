...
- **مصدر القرار الأساسي** هو استعلام GraphQL `fn_is_super_admin_gql` عبر `NhostAuthService._resolveSuperAdminFlag`؛ النتيجة تُدمج مع قائمة `superAdminEmails` القادمة من `AppConstants` كحالة احتياطية فقط إذا فشل الاستعلام أو أرجع `false`. أي خطأ في كشف الدالة أو صلاحياتها يجعل الاعتماد الوحيد على قائمة الإعدادات.
...
> **مخاطر محتملة**: إذا لم تُنشر الدالة `fn_is_super_admin_gql` أو لم تُحدّث `superAdminEmails` في ملف الإعداد (config.json)، فلن يُتعرّف على السوبر أدمن وستُحجب شاشة الإدارة وعمليات إنشاء الحسابات.
...
1. تأكد من نشر `fn_is_super_admin_gql` وأن البريد موجود في `superAdminEmails` لتجنب تجاهل حساب السوبر أدمن.
