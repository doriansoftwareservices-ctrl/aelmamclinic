...
GRANT EXECUTE ON FUNCTION public.fn_is_super_admin_email(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_is_super_admin_email(text) TO service_role;
...
GRANT EXECUTE ON FUNCTION public.admin_sync_super_admin_emails(text[]) TO service_role;
...
DECLARE
  ...
  v_is_super boolean := coalesce(public.fn_is_super_admin(), false)
    ...
    OR public.fn_is_super_admin_email(auth.jwt()->>'email');
  v_allowed text[];
