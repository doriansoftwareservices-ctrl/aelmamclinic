...
async function isSuperAdminUser(
  ...
  if (error && error.code !== "PGRST116") {
    throw new Error(`[admin__freeze_clinic] super_admins lookup failed: ${error.message}`);
  }
  return !!data;
}
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    try {
      ...
      return json({ ok: false, message: "super_admins lookup failed", details: detail }, 500);
    }
