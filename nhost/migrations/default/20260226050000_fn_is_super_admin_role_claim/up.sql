-- Allow superadmin role claim to pass fn_is_super_admin()
...
DECLARE
  ...
  v_role_direct text := current_setting('x-hasura-role', true);
  ...
  v_claim_role text := '';
BEGIN
  -- Admin-secret sessions (if available)
  IF v_hasura_user = 'admin' THEN
    RETURN true;
  END IF;

  -- Direct role claim
  IF v_role_direct IS NOT NULL AND lower(v_role_direct) = 'superadmin' THEN
    RETURN true;
  END IF;
