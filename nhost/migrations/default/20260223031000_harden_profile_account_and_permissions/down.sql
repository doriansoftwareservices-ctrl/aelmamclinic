...
-- Revert to allow-all default behavior (legacy) and previous profile updates.
...
BEGIN
  ...
  IF v_cnt = 0 THEN
    RETURN QUERY SELECT p_account, true, array[]::text[], true, true, true;
    RETURN;
  END IF;
...
DO $m$
BEGIN
  -- Restore legacy profile overwrite behavior.
  EXECUTE $sql$
  ...
  ELSE
    RAISE NOTICE 'skip owner_create_employee_within_limit revert: type owner_create_employee_result not present';
  END IF;
END;
$m$;
