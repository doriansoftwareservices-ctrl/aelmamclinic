-- Stage 3: normalize RPC return types for Hasura tracking.
-- - account_is_paid returns table-backed boolean
-- - self_create_account returns v_uuid_result
-- - create_subscription_request returns v_uuid_result
-- - chat_admin_start_dm returns v_uuid_result

BEGIN;

-- 1) Boolean result view for GraphQL-safe return types.
CREATE OR REPLACE VIEW public.v_bool_result AS
...
SET search_path = public
AS $$
  SELECT public.account_is_paid(p_account) AS account_is_paid;
$$;
