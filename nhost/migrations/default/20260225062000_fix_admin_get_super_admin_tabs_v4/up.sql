...
-- Ultra-hardened tabs resolver: multiple session sources, no exceptions, no unwanted widening.
...
BEGIN
  ...
  -- Must be a superadmin role (to avoid widening access).
  IF v_role <> 'superadmin' THEN
    RETURN;
  END IF;

  -- Verify user is registered as super admin by uid/email.
  ...
  -- Resolve uid from email if missing.
