...
-- Replace create_subscription_request with audit_logs compatible columns.
DROP FUNCTION IF EXISTS public.create_subscription_request(json, text, uuid, text, text, text, text);
...
BEGIN
  ...
  -- Use audit_logs row_pk + after_row (row_id/changes are not valid columns).
