...
BEGIN
  ...
  IF public.fn_is_super_admin() IS NOT TRUE
    ...
    RETURN QUERY SELECT false, 'forbidden', v_account, v_uid, NULL::uuid, v_role, NULL::boolean, NULL::boolean;
    RETURN;
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 'update_failed', v_account, v_uid, NULL::uuid, v_role, NULL::boolean, NULL::boolean;
    RETURN;
  END IF;

  RETURN QUERY SELECT true, NULL::text, v_account, v_uid, NULL::uuid, v_role, NULL::boolean, NULL::boolean;
END;
$$;
