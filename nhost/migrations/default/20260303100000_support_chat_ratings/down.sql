-- Revert support_status column and restore v_chat_conversations_for_me without it
...
BEGIN
  IF to_regclass('public.chat_conversations') IS NULL
    ...
    RAISE NOTICE 'skip chat view revert: chat tables not present';
    RETURN;
  END IF;
...
BEGIN
  ...
  IF EXISTS (
    ...
    EXECUTE 'ALTER TABLE public.chat_conversations DROP CONSTRAINT chat_conversations_support_status_chk';
  END IF;
  ...
  IF EXISTS (
    ...
    EXECUTE 'ALTER TABLE public.chat_conversations DROP COLUMN support_status';
  END IF;
END;
$m$;
