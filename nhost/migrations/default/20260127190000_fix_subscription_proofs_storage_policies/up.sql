-- Ensure storage.files policies exist for subscription-proofs bucket.
-- Idempotent and Nhost-safe (best-effort role elevation).
...
BEGIN
  ...
  IF NOT has_storage_role THEN
    BEGIN
      ...
      has_storage_role := pg_has_role(current_user, 'nhost_storage_admin', 'member');
    EXCEPTION WHEN others THEN
      RAISE NOTICE 'skip storage.files policies: cannot GRANT role to %', current_user;
    END;
  END IF;
  ...
  EXCEPTION WHEN others THEN
    RAISE NOTICE 'skip storage.files policies: cannot SET ROLE (current_user=%)', current_user;
    RETURN;
  END;
