...
-- Align participant/read account scopes with the owning conversation when the
-- user is actually a member of that account. This repairs older rows that were
-- inserted without account_id and were therefore invisible to realtime scope.
UPDATE public.chat_participants AS p
SET account_id = c.account_id
FROM public.chat_conversations AS c
WHERE p.conversation_id = c.id
  AND c.account_id IS NOT NULL
  AND p.account_id IS DISTINCT FROM c.account_id
  AND EXISTS (
    SELECT 1
...
UPDATE public.chat_reads AS r
SET account_id = c.account_id
FROM public.chat_conversations AS c
WHERE r.conversation_id = c.id
  AND c.account_id IS NOT NULL
  AND r.account_id IS DISTINCT FROM c.account_id
  AND EXISTS (
    SELECT 1
    FROM public.account_users AS au
    WHERE au.account_id = c.account_id
      AND au.user_uid = r.user_uid
  );
...
BEGIN
  ...
  INSERT INTO public.chat_participants(
    account_id, conversation_id, user_uid, email, joined_at, role
  )
  VALUES
    (
      v_account_id,
  ...
  ON CONFLICT (conversation_id, user_uid) DO UPDATE
    SET account_id = COALESCE(EXCLUDED.account_id, public.chat_participants.account_id),
...
BEGIN
  ...
  INSERT INTO public.chat_participants(account_id, conversation_id, user_uid, role, email, joined_at)
  VALUES
    (NULL, conv_id, caller_uid, 'superadmin', NULLIF(caller_email, ''), now_ts),
    (target_account, conv_id, target_uid, NULL, normalized_email, now_ts);
