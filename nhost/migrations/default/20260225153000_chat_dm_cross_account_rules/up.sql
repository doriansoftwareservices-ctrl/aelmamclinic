...
-- Enforce: 
-- - Same account: all roles can DM each other.
-- - Cross account: only owners/admins can be targets.
-- - Employees can DM owners/admins across accounts (not employees).
-- - Owners/admins cannot DM employees across accounts.
-- - Superadmin rules unchanged.
...
BEGIN
  ...
  -- Employees: allow lookup داخل الحساب، أو ملاك/أدمن خارج الحساب فقط
  IF NOT v_is_super AND lower(coalesce(v_role,'')) = 'employee' THEN
    ...
    IF v_ident ~ '^555[0-9]{4}$' THEN
      ...
      WHERE au.chat_code = v_ident
        AND (
          au.account_id = v_account OR lower(coalesce(au.role,'')) IN ('owner','admin')
        )
      ORDER BY au.created_at DESC
      LIMIT 1;
      RETURN;
  ...
  -- Owners/Admins/Superadmin: resolve by email/code, but cross-account only to owner/admin.
  RETURN QUERY
  SELECT au.user_uid,
         coalesce(u.email, au.email) AS email,
         au.account_id,
  ...
  FROM public.account_users au
  LEFT JOIN auth.users u ON u.id = au.user_uid
  WHERE (lower(coalesce(au.email,'')) = v_ident OR au.chat_code = v_ident)
    AND (
      v_account IS NULL
      OR au.account_id = v_account
      OR lower(coalesce(au.role,'')) IN ('owner','admin')
    )
  ORDER BY au.created_at DESC
  LIMIT 1;
END;
...
BEGIN
  ...
  SELECT account_id, role INTO v_me_acc, v_me_role
  FROM public.account_users
  WHERE user_uid = v_me
  ORDER BY created_at DESC
  LIMIT 1;

  SELECT account_id, role INTO v_other_acc, v_other_role
  FROM public.account_users
  ...
  IF lower(coalesce(v_me_role, '')) = 'employee' THEN
    IF v_other_is_super THEN
      RAISE EXCEPTION 'superadmin dm forbidden';
    END IF;
    IF v_me_acc IS NULL OR v_other_acc IS NULL THEN
      RAISE EXCEPTION 'cross-account dm forbidden';
    END IF;
    IF v_me_acc <> v_other_acc THEN
      IF lower(coalesce(v_other_role,'')) NOT IN ('owner','admin') THEN
  ...
  IF lower(coalesce(v_me_role, '')) IN ('owner','admin') THEN
    IF v_me_acc IS NOT NULL AND v_other_acc IS NOT NULL AND v_me_acc <> v_other_acc THEN
      IF lower(coalesce(v_other_role,'')) NOT IN ('owner','admin') THEN
        RAISE EXCEPTION 'cross-account owner dm forbidden';
      END IF;
    END IF;
  END IF;
