...
-- Ensure "superadmin" is not a default role for newly created users.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    UPDATE auth.roles
    SET is_default = CASE
      WHEN role IN ('user', 'anonymous') THEN true
      ELSE false
    END
...
-- Remove accidental superadmin role assignments for non-super-admin users.
DO $$
BEGIN
