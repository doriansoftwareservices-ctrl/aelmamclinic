...
-- ROLE-1: Align JWT/Hasura roles with metadata.
-- Current implementation sets default_role/roles to owner/employee, while Hasura metadata permissions are primarily for user/me.
-- This patch forces Hasura roles to:
--   - default_role: 'user' for everyone
--   - allowed roles: ['user'] or ['user','superadmin'] for super admins
-- Domain role (owner/employee) remains stored in user metadata (role) and is enforced via RLS on public.account_users.
...
DECLARE
  ...
  v_meta jsonb := jsonb_strip_nulls(jsonb_build_object('role', v_domain_role, 'account_id', v_account));
...
BEGIN
  ...
  -- Detect super admin by explicit role OR membership in public.super_admins.
  ...
  -- 1) Hasura roles (JWT claims) => auth.users.default_role + auth.users.roles
  IF EXISTS (
    SELECT 1
  ...
  -- 2) Domain metadata (for app logic / RLS helpers)
  IF EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema = 'auth'
