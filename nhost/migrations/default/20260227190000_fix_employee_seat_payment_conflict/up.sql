...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'employee_seat_payments_request_uq'
      AND conrelid = 'public.employee_seat_payments'::regclass
  ) THEN
    ALTER TABLE public.employee_seat_payments
      ADD CONSTRAINT employee_seat_payments_request_uq UNIQUE (request_id);
  END IF;
END $$;
