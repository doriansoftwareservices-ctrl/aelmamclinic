...
DECLARE
  ...
  has_raw_app boolean := false;
  has_raw_user boolean := false;
BEGIN
  ...
  IF has_raw_app OR has_raw_user THEN
    UPDATE auth.users
       SET raw_app_meta_data = CASE
             WHEN has_raw_app
               THEN COALESCE(raw_app_meta_data, '{}'::jsonb) || jsonb_build_object(
                      'role', 'owner',
           ...
           raw_user_meta_data = CASE
             WHEN has_raw_user
               THEN COALESCE(raw_user_meta_data, '{}'::jsonb) || jsonb_build_object(
                      'role', 'owner',
                      'account_id', v_account::text
                    )
